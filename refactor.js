...
// The helpers are everything up to `function canManageKits`
const helpersEndIdx = original.indexOf('function canManageKits(req) {');
const helpersCode = original.substring(0, helpersEndIdx);

const serviceCode = `const { all, get, run, pool } = require("../db.pg");

${helpersCode.substring(helpersCode.indexOf('const ESCUELA_TIPOS'))}
// Business logic
...
fs.writeFileSync('backend/src/services/pedidoService.js', serviceCode);

const controllerCode = \`const pedidoService = require('../services/pedidoService');
...
fs.writeFileSync('backend/src/controllers/pedidoController.js', controllerCode);
...
fs.writeFileSync('backend/src/routes/pedidos.js', routeCode);
