...
with open('backend/src/routes/pedidos.js', 'r', encoding='utf-8') as f:
    original = f.read()

helpers_end = original.find('function canManageKits(req) {')
helpers_code = original[:helpers_end]

# Extract from ESCUELA_TIPOS onwards
helpers_code = helpers_code[helpers_code.find('const ESCUELA_TIPOS'):]

service_code = f"""const {{ all, get, run, pool }} = require("../db.pg");
...
controller_code = """const pedidoService = require('../services/pedidoService');
...
route_code = """const express = require("express");
...
with open('backend/src/services/pedidoService.js', 'w', encoding='utf-8') as f:
    f.write(service_code)

with open('backend/src/controllers/pedidoController.js', 'w', encoding='utf-8') as f:
    f.write(controller_code)

with open('backend/src/routes/pedidos.js', 'w', encoding='utf-8') as f:
    f.write(route_code)
